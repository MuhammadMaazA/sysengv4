...
class InvertedPendulum:
    ...
    def dynamics(self, y, controller_type="PID"):
        ...
        if self.filter_enabled:
            self.filter_history.append([x_meas, x_dot_meas, theta_meas, theta_dot_meas])
        ...
        if controller_type == "PID":
            ...
            u = self.nonlinear_controller.compute_control(theta_meas, theta_dot_meas, self.m, self.L, self.g)
        else:
            u = 0.0
        ...
        x_ddot = ( total_force
                   + gravity_effect
                   - (damping + extra_damping)*x_dot ) / denom

        pivot_damp = self.bd * theta_dot
        theta_ddot = ( -total_force*cos_th
                       - m*L*(theta_dot**2)*sin_th*cos_th
                       - (M+m)*g*sin_th
                       + damping*x_dot*cos_th ) / (L*denom)
